  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Semua aktivitas peminjaman buku</p>
      </div>

      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {[
          { label: 'Total Peminjaman', val: transactions.filter(t => t.status !== 'pending').length, color: 'var(--primary)', icon: BookOpen },
          { label: 'Terlambat', val: transactions.filter(t => t.status === 'late').length, color: '#ef4444', icon: AlertTriangle },
      ...
      {loading ? (
        ...
        <div className="card empty-state"><BookOpen size={40} /><h3>Belum ada riwayat</h3></div>
      ) : (
            ...
            return (
              <div key={t.id} className="card" style={{ padding: '16px 20px' }}>
                <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                  <div style={{ width: '52px', height: '68px', flexShrink: 0, background: 'var(--primary-pale)', borderRadius: '8px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {t.books?.cover_url ? (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px', marginBottom: '4px' }}>
                      <h4 style={{ fontSize: '14px', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.books?.title || '-'}</h4>
                    ...
                    <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '8px' }}>{t.books?.author}</p>
                    ...
                    {currentFine > 0 && (
                      <div style={{ marginTop: '8px', padding: '6px 12px', background: '#fef2f2', borderRadius: '8px', fontSize: '12px', color: 'var(--danger)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
